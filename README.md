...
| **The update UI is DSH's own** | A client plugin renders it inside DSH's General settings, next to every other setting; the shell's own window keeps it as a fallback for when DSH will not start |
